...
function QuemSomos() {
  ...
  return (
    <div className="mx-auto max-w-[1200px] animate-fade-up px-4 py-10 sm:py-12 md:px-8 md:py-16">
      <header className="flex items-center gap-3">
        ...
        <div className="min-w-0">
          <h1 className="text-3xl leading-[1.02] sm:text-4xl md:text-6xl">{aboutTitle}</h1>
      ...
      <section className="mt-10 sm:mt-12">
        ...
        {certificates.length === 0 ? (
          <p className="card-surface mt-6 p-8 text-center text-sm text-muted-foreground">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {certificates.map((c) => (
              <article key={c.id} className="card-surface flex gap-4 p-4">
                {c.image_url ? (
                  <img src={normalizeImage(c.image_url)} alt={c.title} loading="lazy" className="h-20 w-20 shrink-0 rounded-xl border border-border bg-card object-contain p-1" />
                ) : (
...
function RichText({ text }: { text: string }) {
  ...
  return (
    <div className="content-copy space-y-4">
      {blocks.map((block, i) => {
        if (block.startsWith("## ")) {
          return <h2 key={i} className="pt-4 text-2xl font-bold uppercase md:text-3xl">{block.slice(3)}</h2>;
        }
        return (
